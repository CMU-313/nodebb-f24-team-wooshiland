...
describe("Controllers", () => {
	...
	before(async () => {
		...
		testRoutes = [
			...
			{ it: "should load recent posts rss feed", url: `/recentposts.rss` },
			{
				...
				url: `/category/${cid}/recentposts.rss`,
			},
			...
			{ it: "should load OH Queue page", url: `/oh-queue` },
		...
		describe("abort behaviour", () => {
			...
			it("should terminate the session and send user back to index if interstitials remain", async () => {
				...
				assert.strictEqual(
					...
					`express.sid=; Path=${nconf.get("relative_path") || "/"}; Expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Lax`,
				);
				assert.strictEqual(
					response.headers.location,
					`${nconf.get("relative_path")}/`,
	...
	describe("composer", () => {
		...
		it("should load the composer route", async () => {
			const { response, body } = await request.get(
				`${nconf.get("url")}/api/compose?cid=1`,
			);
			assert.equal(response.statusCode, 200);
			assert(body.title);
